...
export function LoopVisualization({ phases, currentPhase, isRunning }: Props) {
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-3 sm:p-5 h-full">
      ...
      <div className="mt-4 pt-4 border-t border-[var(--border)]">
        <div className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
          {isRunning ? (
            <>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2" className="animate-spin-slow">
                <path d="M3 12a9 9 0 1 0 9-9" />
                <path d="M3 4v5h5" />
              </svg>
              <span className="text-[var(--success)]">Loop running — {phases.filter(p => p.status === 'done').length}/7 phases complete</span>
            </>
          ) : (
            <>
              ...
              <span>Loop idle — awaiting next scheduled run</span>
            </>
          )}
        </div>
      </div>
